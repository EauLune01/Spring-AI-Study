...
public class TravelOrchestrator {
  ...
  /**
   * ThreadLocal: 각 스레드가 자기만의 독립적인 변수를 갖는 것
   * InheritableThreadLocal: 부모 스레드의 값을 자식 스레드가 상속받는 것
   * SSE는 실시간 스트리밍 응답 채널인데, Tool이 비동기로 실행될 때 별도 스레드에서도 같은 SseEmitter에 접근해서
   * 클라이언트로 중간 결과를 쏴줘야 하니까 InheritableThreadLocal을 쓰는 것
   *
   * */
  ...
  // ----------------------------------------------------------------------------------------
  /**
   * LLM의 역할 및 도구 호출, 응답 규칙을 설정
   *  이전 대화의 흐름을 기억하며 불완전한 사용자의 질문을 이전 대화 맥락을 통해 이해하도록 구성
   */
  ...
  private void sendSseEvent(String agentName, String status, String message) {
    try {
      SseEmitter emitter = threadLocal.get(); // 현재 스레드의 SSE 채널 꺼내기
      if (emitter != null) {
        emitter.send(
                SseEmitter.event()
                        .name("agent")           // 이벤트 타입 이름 (프론트에서 구분용)
                        .data(Map.of(
                                "agent", agentName,  // 어떤 에이전트인지  ex) "YoutubeAgent"
                                "status", status,    // 상태              ex) "START", "DONE", "ERROR"
                                "message", message   // 메시지 내용        ex) "유튜브 검색 중..."
                        ))
        );
      }
    } catch (Exception e) {
  ...
  /**
   * @Tool 메소드: 해당 에이전트를 실행시키는 여갈, 즉 도구를 호출한다는 것은 에이전트를 실행하는 것과 같은 의미
   * returnDirect=true: 도구 메소드의 반환값을 다시 LLM으로 보내지 않도록 사용,
   * 실제 처리는 에이전트가 하는데(에이전트가 준 완벽한 형식의 List<T> 결과를 다시 LLM으로 보낼 필요 X
   * */
