...
public class AiService {
  ...
  public Flux<String> imageAnalysis(String question, String contentType, byte[] bytes) {
    ...
    // UserMessage 생성 시 텍스트 질문과 Media를 포함 (멀티모달리티)
  ...
  // ##### 이미지를 새로 생성하는 메소드 #####
  //input: text->output: Base64 encoded image String
  public String generateImage(String description) {
    ...
    // 프롬프트 생성(imageMessage과 ImageOptions 활용)
  ...
  // ##### 원본 이미지를 편집하는 메소드 #####
  //input: text+ original image + mask image
  /**
   * OpenAI의 API로 원본 이미지, 마스크 이미지 보낼 때는 멀티파트 폼으로 보내야 함->파일명이 필요
   * **/
